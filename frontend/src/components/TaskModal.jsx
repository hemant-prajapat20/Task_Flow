...
const TaskModal = ({ isOpen, onClose, onSubmit, task, boardId }) => {
  ...
  const suggestEstimate = async () => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        estimatedEffort: data.effort || prev.estimatedEffort,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto">
      <div className="bg-surface-light dark:bg-surface-dark rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 w-full max-w-lg my-8 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-800/50 dark:hover:bg-indigo-700 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-lg transition-colors disabled:opacity-50"
                >
                  {isAiLoading ? (
